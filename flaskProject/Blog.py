...
# Arama tablosunun modeli
class BlogD(db.Model):
    __tablename__ = 'arama'
    query = db.Column(db.Text, primary_key=True)  # Query alanını birincil anahtar olarak ayarladık

# Arama sayfası
@app.route('/arama', methods=['GET'])
def ara():
    sorgu = request.args.get('query')
    if sorgu:
        # Aynı sorgu daha önce eklenmiş mi kontrol et
        existing_query = BlogD.query.filter_by(query=sorgu).first()
        if not existing_query:
            yeni_arama = BlogD(query=sorgu)
            db.session.add(yeni_arama)
            db.session.commit()
        return render_template('ana.html')
    return "Sorgu girilmedi."
